 ...
 *
 * This has been backported from bedrock linux 1.0alpha3 bosco to 1.0alpha2 momo.
 */
...
void ensure_enough_arguments(int argc, char* argv[]){
	if(argc < 2){
		fprintf(stderr, "No client given\n");
		exit(1);
	}
}
...
char** get_chroot_command(int argc, char* argv[], char* shell[]){
	if(argc > 2){
		...
		/*
		 * if no third term is given, we have nothing to run.  instead, run the
		 * $SHELL, if we can find $SHELL.  otherwise, fall back to /bin/sh.
		 */
...
int main(int argc, char* argv[]){
	...
	/*
	 * if the first argument is -l, list the clients
	 * note this is a hold-over from momo and will likely be moved to another
	 * command (maybe "bri") in bosco
	 *
	 * being lazy and calling a shell script since this is a quick-and-dirty backport
	 */
	if(strncmp(argv[1], "-l", 3) == 0){
		system("awk '/^\\[.*\\]$/{print substr($0,2,length($0)-2)}' /opt/bedrock/etc/brclients.conf");
		return 0;
	}
